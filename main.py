        ...
        for imgs, t_labels, d_labels in tqdm(dataloader):
            ...
            with torch.no_grad():
                ...
                for imgs, t_labels, d_labels in validloader:
                    ...
                    valid_loss += alpha * criterion(t_ps, t_labels) + (
                        1 - alpha
                    ) * criterion(d_ps, d_labels)
...
if __name__ == "__main__":
    ...
    if (
        len(valid_losses) > 0
        and isinstance(valid_losses[0], torch.Tensor)
        and valid_losses[0].is_cuda
    ):
        valid_losses = to_cpu_list(valid_losses)
